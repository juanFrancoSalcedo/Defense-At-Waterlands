...
namespace RTSEngine.NPC.UnitExtension
{
    ...
    {
        #region Attributes
        [SerializeField, Tooltip("In case this component or any other NPC component attempts to create a unit that does not have valid regulator data defined and this field is assigned, the regulator defined here will be used for that unit.")]
        private NPCUnitRegulatorData defaultRegulatorData = null;
        ...
        protected override void OnPreInit()
        {
            ...
            if(logger.RequireValid(PopulationResource, $"[{GetType().Name} - Faction ID: {factionMgr.FactionID}] No population resource has been set, only the minimum amount of units will be auto-created!", type: Logging.LoggingType.warning))
                logger.RequireTrue(PopulationResource.HasCapacity && PopulationResource.CapacityType == ResourceCapacityType.slotBased, $"[{GetType().Name} - Faction ID: {factionMgr.FactionID}] Resource '{PopulationResource.Key}' Must be a capacity resource with capacity type set to 'Slot Based' to be used as the main population resource!");
        ...
        public NPCUnitRegulator ActivateUnitRegulator(IUnit unitPrefab)
        {
            ...
            LogEvent($"{unitPrefab.Code}: Created and activated a regulator for unit. Using default regulator data? {regulatorData == defaultRegulatorData}");
        ...
        private void DestroyAllActiveRegulators()
        {
            ...
            LogEvent("All active regulators destroyed!");
        }
        ...
        private void DestroyActiveRegulator(string unitCode)
        {
            ...
            LogEvent($"{unitCode}: Active regulator destroyed!");
        }
        #endregion
        ...
        public bool OnCreateUnitRequest(string unitCode, int requestedAmount, out int createdAmount)
        {
            ...
            if (!nextUnitRegulator.IsValid())
            {
                LogEvent($"{unitCode}: Unable to create {requestedAmount} instances. No valid regulator found!");
                return false;
            }
            else if (nextUnitRegulator.Data.CanCreateOnDemand == false)
            {
                LogEvent($"{unitCode}: Unable to create {requestedAmount} instances. Creation on demand is disabled!");
                return false;
            }
        ...
        private bool OnCreateUnitRequestInternal(NPCUnitRegulator instance, int requestedAmount, out int createdAmount)
        {
            ...
            if(instance.HasReachedMaxAmount
                || requestedAmount <= 0)
            {
                LogEvent($"{instance.Prefab.Code}: Unable to create {requestedAmount} instances. Has reached max amount? {instance.HasReachedMaxAmount}");
                return false;
            }
            ...
            {
                LogEvent($"{instance.Prefab.Code}: Unable to create {requestedAmount} instances. No valid UnitCreator found!");
            ...
            LogEvent($"{instance.Prefab.Code}: Attempting to create {requestedAmount+createdAmount} instances, successfully launched {createdAmount} creation tasks!");
