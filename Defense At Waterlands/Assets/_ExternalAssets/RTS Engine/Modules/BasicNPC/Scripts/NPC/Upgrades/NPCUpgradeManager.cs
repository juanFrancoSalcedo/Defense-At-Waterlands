...
namespace RTSEngine.NPC.Upgrades
{
    ...
    public class NPCUpgradeManager : NPCComponentBase, INPCUpgradeManager
    {
        ...
        private struct UpgradeLog
        {
            ...
            public UpgradeLauncher upgradeLauncher;
        }

        [SerializeField, ReadOnly]
        private UpgradeLog[] upgradeLogs = new UpgradeLog[0];

        protected override void UpdateActiveLogs()
        {
            upgradeLogs = npcTracker.UpgradeLauncherTracker
                .Components
                .Select(comp => new UpgradeLog
                {
                    entity = comp.Entity.gameObject,
                    upgradeLauncher = (UpgradeLauncher)comp
                })
                .ToArray();
        }
        #endregion
