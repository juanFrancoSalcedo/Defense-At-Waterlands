...
namespace RTSEngine.ResourceExtension
{
    public class ResourceManager : MonoBehaviour, IResourceManager
    {
        ...
        public void UpdateResource(int factionID, ResourceInput resourceInput, bool add)
            => UpdateResource(factionID, resourceInput, add, out _);
        ...
        /// <param name="resourceInput">Defines the resource type and amount to add/remove.</param>
        /// <param name="add">Adds the resources when true, otherwise removes the resources from the faction.</param>
        /// <param name="restAmount">In case of a capacity type resource with limit based capacity, this returns the amount that can not be added due to reaching maximum capacity, if that maximum capacity is ever reached.</param>
        public void UpdateResource(int factionID, ResourceInput resourceInput, bool add, out int restAmount)
        {
            ...
            resourceHandler.UpdateAmount(updateValue, out restAmount);
        }
        ...
        public void SetResource(int factionID, ResourceInput resourceInput)
        {
            ...
            resourceHandler.SetAmount(resourceInput.value, out _);
        }
        ...
        public bool HasResources(ResourceInput resourceInput, int factionID)
        {
            ...
            return
                ...
                (resourceHandler.Type.HasCapacity && resourceHandler.Type.CapacityType == ResourceCapacityType.slotBased
        ...
        public bool HasResourceTypeReachedLimitCapacity (ResourceTypeInfo resourceType, int factionID)
        {
            return resourceType.IsValid()
                && resourceType.HasCapacity
                && resourceType.CapacityType == ResourceCapacityType.simpleLimit
                && FactionResources[factionID].ResourceHandlers[resourceType].FreeAmount == 0;
        }
        #endregion
    }
