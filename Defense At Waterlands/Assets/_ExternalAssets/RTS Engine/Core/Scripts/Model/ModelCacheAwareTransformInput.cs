...
namespace RTSEngine.Model
{
    ...
    public class ModelCacheAwareRendererInput : ModelCacheAwareInput<Renderer>
    {
        ...
        #region Material
        public bool SetMaterial(int materialID, Material material)
        {
            switch(status)
            {
                case ObjectToModelStatus.notChild:
                    ...
                    obj.materials[materialID] = material;
                    return true;
                case ObjectToModelStatus.child:
                    return EntityModel.GetRendererChild(indexKey).SetMaterial(materialID, material);
