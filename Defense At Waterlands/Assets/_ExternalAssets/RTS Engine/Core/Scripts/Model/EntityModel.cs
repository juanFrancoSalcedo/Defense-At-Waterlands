...
namespace RTSEngine.Model
{
    public class EntityModel : MonoBehaviour, IEntityModel 
    {
        ...
        // True when the EntityModel is initialized on a pre-placed entity in the current map scene
        private bool isPrePlacedEntity = false;
        private Vector3 prePlacedLocalPosition;
        private Quaternion prePlacedLocalRotation;
        private Vector3 prePlacedLocalScale;
        ...
        public void OnPrefabInit(IEntity entity, IGameManager gameMgr)
        {
            ...
            isPrePlacedEntity = false;
        }

        public void OnPrePlacedInit(EntityModelConnections model)
        {
            // For pre-placed entities in the scene, we need to preserve the local Transform component
            isPrePlacedEntity = true;

            prePlacedLocalPosition = model.transform.localPosition;
            prePlacedLocalRotation = model.transform.localRotation;
            prePlacedLocalScale = model.transform.localScale;
        }
        ...
        private void GenerateChildrenHandlers ()
        {
            ...
            transformHandlers = modelObject.TransformConnections
                ...
                .Select(child =>
                {
                    ...
                    // If this was a pre-placed entity and this is the main model object
                    // Assign the pre-placed local Transform properties to the transform handler
                    if(isPrePlacedEntity && child == modelObject.transform)
                    {
                        newHandler.LocalPosition = prePlacedLocalPosition;
                        newHandler.LocalRotation = prePlacedLocalRotation;
                        newHandler.LocalScale = prePlacedLocalScale;
                    }
