    ...
    public class EntityUpgradeManager : MonoBehaviour, IEntityUpgradeManager
    {
        ...
        private void UpgradeInstance(IFactionEntity sourceInstance, UpgradeElement<IEntity> upgradeElement, int factionID, IEnumerable<EntityUpgradeComponentMatcherElement> entityComponentMatcher, IEffectObject upgradeEffect)
        {
            ...
            if (sourceInstance.IsBuilding())
            {
                ...
                // If the source building has Border component, then remove all resources under its territory
                // The resource removal would occur normally when the source building is destroyed
                // however, by that time, the same resources would be added to the upgraded building instance
                // and removing them later will mean that they be removed from the new upgraded building instance border as well
                if (currBuilding.BorderComponent.IsValid())
                    currBuilding.BorderComponent.Disable();
